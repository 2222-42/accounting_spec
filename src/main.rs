...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 3. Create Sections
    let section_a = Section::new(
        "Sales Dept A".to_string(),
        SectionType::Section,
        None,
    );
    let section_b = Section::new(
        "Sales Dept B".to_string(),
        SectionType::Section,
        None,
    );
    ...
    println!("Created Sections: A ({}), B ({})", section_a_id, section_b_id);
    ...
    let date = NaiveDate::from_ymd_opt(2025, 6, 15).unwrap().and_hms_opt(10, 0, 0).unwrap();
    ...
    println!("Transferred Sales {} to Section B. New Sales ID: {}", sales_id, new_sales_id);
    ...
    let correction_date = NaiveDate::from_ymd_opt(2025, 12, 31).unwrap().and_hms_opt(23, 59, 59).unwrap();
    
    service.rebalance_term(term_id, section_b_id, section_a_id, correction_amount, correction_date)?;
